/**
 * KiroTextGeneration — text generation via Kiro CLI ACP runtime.
 *
...
import { type KiroSettings, type ModelSelection, TextGenerationError } from "@t3tools/contracts";
  ...
  const runKiroJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      ...
      const acpLayer = AcpSessionRuntime.layer({
        ...
        Layer.provide(Layer.succeed(ChildProcessSpawner.ChildProcessSpawner, commandSpawner)),
      );

      const acpContext = yield* Layer.build(acpLayer);
      const runtime: AcpSessionRuntimeShape = yield* Effect.service(AcpSessionRuntime).pipe(
            ...
            onNone: () =>
              Effect.fail(
                new TextGenerationError({
                  operation,
                  detail: "Kiro CLI request timed out.",
                }),
              ),
      ...
      return yield* decodeOutput(extractJsonObject(rawResult)).pipe(
        Effect.catchTag("SchemaError", (cause) =>
          Effect.fail(
            new TextGenerationError({
              operation,
              detail: "Kiro CLI returned invalid structured output.",
              cause,
            }),
          ),
        ),
